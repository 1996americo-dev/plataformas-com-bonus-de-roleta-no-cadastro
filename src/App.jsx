...
export default function App(){
  ...
  async function carregar(){
    const { data } = await supabase.from('links_roleta').select('*').order('ordem',{ascending:true})
    ...
    return (
      <div style={{maxWidth:480,margin:'0 auto',padding:16,color:'#fff'}}>
        ...
        <div style={{marginTop:12}}>
          {links.map(l=>(
            <div key={l.id} style={{display:'flex',justifyContent:'space-between',background:'#1a',padding:8,borderRadius:8,marginBottom:6,fontSize:11}}>
              <span>{l.categoria==='principal'?'⭐':''} {l.titulo}</span>
              <button onClick={()=>deletar(l.id)} style={{background:'#ef4444',color:'#fff',border:'none',borderRadius:4,padding:'2px 6px'}}>X</button>
            </div>
          ))}
        </div>
        <button onClick={()=>{location.hash='';location.href='/'}} style={{width:'100%',marginTop:12,padding:8,background:'#222',color:'#fff',border:'none',borderRadius:8}}>Ver site</button>
      </div>
    )
  }
    ...
    <div style={{minHeight:'100vh',background:'#070708',display:'flex',justifyContent:'center',padding:14}}>
      <div style={{width:'100%',maxWidth:900}}>
        <div style={{textAlign:'center',padding:'16px 0 8px'}}>
          <div style={{width:110,height:110,margin:'0 auto 12px',borderRadius:'50%',background:'#0f172a',border:'3px solid #fbbf24',display:'flex',alignItems:'center',justifyContent:'center',fontSize:50,boxShadow:'0 25px rgba(251,191,36,0.6)'}}>🎰</div>
